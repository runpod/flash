...
def _generate_flash_server(project_root: Path, workers: List[WorkerInfo]) -> Path:
    ...
    # set name column width for aligned dev console output
    resource_names = [w.resource_name for w in workers]
    lines += [
        "from runpod_flash.dev_console import set_name_width as _set_name_width",
        f"_set_name_width({resource_names!r})",
        "",
        "app = FastAPI(",
