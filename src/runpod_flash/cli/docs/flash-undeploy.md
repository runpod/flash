...
The `flash undeploy` command helps you clean up Serverless endpoints that Flash has created when you deployed `Endpoint` functions using `flash run` or `flash deploy`. It manages endpoints recorded in `.flash/resources.pkl` and ensures both the cloud resources and local tracking state stay in sync.
